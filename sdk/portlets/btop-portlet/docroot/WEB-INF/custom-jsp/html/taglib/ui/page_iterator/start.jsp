...
int maxPages = 2;//GetterUtil.getInteger((String)request.getAttribute("liferay-ui:page-iterator:maxPages"));
...
<c:if test='<%= type.equals("approximate") || type.equals("more") || type.equals("regular") || ((type.equals("article") || type.equals("aggregator")) && (total > resultRowsSize)) %>'>
...
<c:if test="<%= (total > delta) || (total > PropsValues.SEARCH_CONTAINER_PAGE_DELTA_VALUES[0]) %>">
	<div class="search-pages">
		...
		<div class="page-links">
			...
			<c:choose>
				<c:when test="<%= cur != 1 %>">
					<a class="<%= type.equals("aggregator") ? "btn-" : "" %>previous" href="<%= _getHREF(formName, curParam, cur - 1, jsCall, url, urlAnchor) %>" target="<%= target %>">
				</c:when>
				<c:when test='<%= type.equals("approximate") || type.equals("more") || type.equals("regular") %>'>
					<span class="previous">
				</c:when>
				<c:when test='<%= type.equals("aggregator") %>'>
					<span class="btn-previous">
				</c:when>
			</c:choose>
			
			<c:choose>
				<c:when test='<%= type.equals("aggregator") && cur != 1  %>'>
					<liferay-ui:message key="previous" />
				</c:when>
				<c:when test='<%= (type.equals("approximate") || type.equals("more") || type.equals("regular") || cur != 1) %>'>
					<liferay-ui:message key="previous" />
				</c:when>
			</c:choose>
			...
			<c:choose>
				...
				<c:when test='<%= type.equals("approximate") || type.equals("more") || type.equals("regular") || type.equals("aggregator") %>'>
					</span>
				</c:when>
			</c:choose>
			
			<c:if test='<%= (type.equals("article") || type.equals("aggregator")) && (total > resultRowsSize) %>'>
			...
			<c:choose>
				<c:when test="<%= cur != pages %>">
					<a class="<%= type.equals("aggregator") ? "btn-" : "" %>next" href="<%= _getHREF(formName, curParam, cur + 1, jsCall, url, urlAnchor) %>" target="<%= target %>">
				</c:when>
				<c:when test='<%= type.equals("approximate") || type.equals("more") || type.equals("regular") %>'>
					<span class="next">
				</c:when>
...
private String _getHREF(String formName, String curParam, int cur, String jsCall, String url, String urlAnchor) throws Exception {
	...
	if (Validator.isNotNull(url)) {
		java.util.regex.Pattern p = java.util.regex.Pattern.compile("(" + curParam + "=)" + "(.*?)" + "([&#$])");
		java.util.regex.Matcher m = p.matcher(url);
		if (m.find()) {
			String urlFirstPart = url.substring(0, m.start(2));
			String urlLastPart = url.substring(m.end(2));
			href = HtmlUtil.escape(urlFirstPart + cur + urlLastPart);
		} else {
